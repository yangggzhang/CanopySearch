...
class AnalyticInverseKinematics():
    def __init__(self):
        # robot parmas
        # ## all these values from CAD model
        self.l1 = 41.25 * 10**(-3) + 0.072
        self.l2 = math.sqrt(0.2**2 + 0.05**2)
        self.l3 = 200 * 10**(-3)
        # TODO: Add l4 distance with gripper as well
        self.l4 = (63) * 10**(-3)  # without gripper
        self.angle2_bias = math.atan2(0.05, 0.2)
        self.joint_limits = ([-pi, pi],
                             [-pi, pi],
                             [-pi, pi],
                             [-pi, pi])
    ...
    def ik(self, x, y, z, alpha=0.0, cur_arm_config = 4*[0]):
        """
        ...
        angle1 = (math.atan2(inter1[0], inter1[1] - self.l1) - self.angle2_bias,
                  math.atan2(inter2[0], inter2[1] - self.l1) - self.angle2_bias)

        angle2 = (-math.atan2(p3[1] - inter1[1], p3[0] - inter1[0]) - angle1[0],
                  -math.atan2(p3[1] - inter2[1], p3[0] - inter2[0]) - angle1[1])

        angle3 = (-alpha - angle1[0] - angle2[0],
                  -alpha - angle1[1] - angle2[1])
        ...
        if all(self.joint_limits[1][0] < i < self.joint_limits[1][1] for i in angle1) and \
                all(self.joint_limits[2][0] < i < self.joint_limits[2][1] for i in angle2) and \
                all(self.joint_limits[3][0] < i < self.joint_limits[3][1] for i in angle3):
            # take the one which is closer to the current arm configuration
            diff_0 = abs(angle1[0] - cur_arm_config[1]) + abs(angle2[0] - cur_arm_config[2]) \
                     + abs(angle3[0] - cur_arm_config[3])
            diff_1 = abs(angle1[1] - cur_arm_config[1]) + abs(angle2[1] - cur_arm_config[2]) \
        ...
        elif self.joint_limits[1][0] < angle1[0] < self.joint_limits[1][1] and \
                self.joint_limits[2][0] < angle2[0] < self.joint_limits[2][1] and \
                self.joint_limits[3][0] < angle3[0] < self.joint_limits[3][1]:
            theta[1] = angle1[0]
            theta[2] = angle2[0]
            theta[3] = angle3[0]
            return theta

        elif self.joint_limits[1][0] < angle1[1] < self.joint_limits[1][1] and \
                self.joint_limits[2][0] < angle2[1] < self.joint_limits[2][1] and \
                self.joint_limits[3][0] < angle3[1] < self.joint_limits[3][1]:
            theta[1] = angle1[1]
            theta[2] = angle2[1]
            theta[3] = angle3[1]
            return theta
    ...
    def ik_quat(self, trans, quat, current_config=4 * [0]):
        """
        ...
        return self.ik(trans, -pitch, current_config)
